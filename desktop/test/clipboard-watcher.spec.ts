...
function makeWatcher(overrides: {
  ...
  let snapshot: ClipboardSnapshot = { text: '', formats: ['text/plain'] };
...
describe('ClipboardWatcher', async () => {
  it('captures each new clipboard value once', async () => {
    ...
    expect(h.captured.map((c) => c.text)).toEqual(['first', 'second']);
  ...
  it('does not capture while disabled, and does not replay on re-enable', async () => {
    ...
    await h.poll();
    expect(h.captured.map((c) => c.text)).toEqual(['copied while on']);
  });
  ...
  it('skips likely secrets but marks captured ones when opted in', async () => {
    ...
    on.set('123456');
    await on.poll();
    expect(on.captured).toEqual([{ text: '123456', potentialSecret: true }]);
  });
});
